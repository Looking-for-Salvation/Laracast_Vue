	...
	>
		...
		<div class="flex flex-col items-center justify-between h-full body md:items-stretch md:w-3/4 lg:w-full">
			<div class="mb-4 text-center md:text-right" style="max-width: 650px;">
				...
				<p class="mb-2 text-sm text-gray-500 lg:text-base clamp three-lines font-IranSans">{{ description }}</p>
			</div>
			<div>
				<div class="flex mb-6 text-gray-400 md:hidden lg:flex expanded-card-meta text-2xs font-IranSans">
					...
					<div class="flex items-center ml-4 expanded-card-meta-lessons text-2xs">
						...
						<router-link :to="url" class="font-IranSans hover:underline">{{ `${lessonsCount} درس` }}</router-link>
					</div>
...
export default {
	...
	setup(props) {
		const categoryObject = computed(() => {
			if (props.category === "framework") return { name: "فریمورک ها", url: "/browse/frameworks" };
			else if (props.category === "language") return { name: "زبان ها", url: "/browse/languages" };
			else if (props.category === "technique") return { name: "تکنیک ها", url: "/browse/techniques" };
			else if (props.category === "testing") return { name: "تست", url: "/browse/testing" };
			else return { name: "ابزار ها", url: "/browse/tooling" };
		});

		const isFrameworks = computed(() => {
			if (props.category === "framework") {
				return true;
			} else {
				return false;
			}
		...
		const difficultyPersian = computed(() => {
			if (props.difficulty === "beginner") return "آسان";
			else if (props.difficulty === "intermediate") return "متوسط";
			else return "پیشرفته";
		});

		const LengthHours = computed(() => {
			if (props.length.hours !== 0) {
				return props.length.hours + "h";
			} else return "";
		});

		const LengthMinutes = computed(() => {
			if (props.length.minutes !== 0) {
				return props.length.minutes + "m";
			} else return "";
		});
