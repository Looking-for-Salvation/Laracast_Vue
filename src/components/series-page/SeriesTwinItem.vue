<template>
	...
	>
		<div class="relative flex flex-col items-center flex-1 lg:flex-row right-side">
			<img :src="item1.imgSource" :alt="item1.name" class="relative mb-8 rounded-lg lg:absolute right-side-thumbnail lg:mb-0 lazyloaded" width="500" />
			<div class="flex flex-col items-center justify-between px-8 lg:px-0 lg:items-stretch right-side-body">
				<div class="flex flex-col flex-1 mb-4 text-center lg:text-right" style="max-width: 500px;">
					<h3 class="font-light text-black font-IranSans text-3lg">
						<router-link class="inherits-color hover:underline" :to="item1.url">{{ item1.name }}</router-link>
					</h3>
					<p class="my-auto mt-5 text-sm font-light text-gray-500 md:text-xs font-IranSans clamp five-lines">{{ item1.description }}</p>
				</div>
				<div>
					<div class="flex justify-around mb-6 text-gray-500 expanded-card-meta text-2xs opacity-70 font-IranSans">
						<div class="flex items-center ml-4 expanded-card-meta-lessons">
							...
							<router-link :to="item1.url" class="inherits-color hover:underline">{{ `${item1.lessonsCount} درس` }}</router-link>
						</div>
		...
		<div class="relative flex flex-col-reverse items-center flex-1 lg:flex-row left-side">
			<img :src="item2.imgSource" :alt="item2.name" class="relative mt-8 rounded-lg lg:absolute left-side-thumbnail lg:mt-0 lazyloaded" width="500" />
			<div class="flex flex-col items-center justify-between px-8 lg:px-0 lg:items-stretch left-side-body">
				<div class="flex flex-col flex-1 mb-4 text-center lg:text-right" style="max-width: 500px;">
					<h3 class="font-light text-black font-IranSans text-3lg">
						<router-link class="inherits-color hover:underline" :to="item2.url">{{ item2.name }}</router-link>
					</h3>
					<p class="my-auto mt-5 text-sm font-light text-gray-500 md:text-xs clamp font-IranSans five-lines">{{ item2.description }}</p>
				</div>
				<div>
					<div class="flex justify-around mb-6 text-gray-500 opacity-70 expanded-card-meta text-2xs font-IranSans">
						<div class="flex items-center ml-4 expanded-card-meta-lessons">
							...
							<router-link :to="item2.url" class="inherits-color hover:underline">{{ `${item2.lessonsCount} درس` }}</router-link>
						</div>
						<div class="flex items-center expanded-card-meta-length">
							<svg width="13" height="13" viewBox="0 0 13 13" class="ml-2 inherits-color">
								<path
...
export default {
	...
	setup(props) {
		const item1LengthHours = computed(() => {
			if (props.item1.length.hours !== 0) {
				return props.item1.length.hours + "h";
			} else return "";
		});

		const item1LengthMinutes = computed(() => {
			if (props.item1.length.minutes !== 0) {
				return props.item1.length.minutes + "m";
			} else return "";
		});

		const item1CalcLength = computed(() => {
			return `${item1LengthHours.value} ${item1LengthMinutes.value}`;
		});

		const item2LengthHours = computed(() => {
			if (props.item2.length.hours !== 0) {
				return props.item2.length.hours + "h";
			} else return "";
		});

		const item2LengthMinutes = computed(() => {
			if (props.item2.length.minutes !== 0) {
				return props.item2.length.minutes + "m";
			} else return "";
		});

		const item2CalcLength = computed(() => {
			return `${item2LengthHours.value} ${item2LengthMinutes.value}`;
		});
